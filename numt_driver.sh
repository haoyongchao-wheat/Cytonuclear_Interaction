...
printf '%s\n' "${SAMPLES[@]}" | xargs -n 1 -P "$CONC" bash -lc '
  python3 07_paf_structural_validation_v1/parse_paf_and_classify.py \
    --type numt \
    --paf-dir 06_minimap2_mapping/numt \
    --position-mapping-csv 04_bed_by_flanking_status_v2/numt_position_mapping.csv \
    --out-dir 07_paf_structural_validation_v1/out \
    --samples "$1" \
    --no-combined \
    > "07_paf_structural_validation_v1/out/logs/numt_${1}.log" 2>&1
' _ 

python3 07_paf_structural_validation_v1/merge_sample_outputs.py \
  --type numt \
  --out-dir 07_paf_structural_validation_v1/out \
  > "$LOGDIR/numt_MERGE.log" 2>&1
